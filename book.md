...
# 2.11. E02521 (O축) IPM 폴트 - Gate 구동전원 저전압
...
*   <전원 LED 확인>
    ...
    ->  Hi6-N : 서보 구동장치의 ‘POW’ LED를 확인하여 주십시오.

    ->  Hi6-T : 백플레인보드(BD602T)의 ‘LED5’ 를 확인하여 주십시오.

    ->  제어전원 공급장치의 CMSMPS에서 ‘DC OK’ LED를 확인하여 주십시오.


*   <보드 LED와 SMPS LED가 모두 OFF된 경우>

    (2)	제어전원 공급장치의 출력을 확인하여 주십시오.

    ->  Hi6-N

    *   BD640의 CN24VB1 커넥터 연결을 분리한 후 PSM의 ‘SMPS OK’ LED를 확인하여 주십시오.

    *   BD640 보드를 분리한 후 서보구동장치의 ‘POW’ LED를 확인하여 주십시오. 

    ->  Hi6-T 

    *   BD602T의 CN24VB1 커넥터 연결을 분리한 후 CMSMPS의 ‘DC OK’ LED를 확인하여 주십시오.

    *   BD641T 보드를 분리한 후 백플레인보드(BD602T)의 ‘LED5’를 확인하여 주십시오. 
    ...
    * PSM과 BD640을 연결하는 CN24VB1 케이블을 교체한 후 LED를 확인하여 주십시오.
    ...
    * CMSMPS와 백플레인보드(BD602T)를 연결하는 CN24VB1 케이블을 교체한 후 LED를 확인하여 주십시오.

    * 서보 보드를 교체한 후 LED를 확인하여 주십시오.

    * 백플레인보드(BD602T)를 교체한 후 LED를 확인하여 주십시오.
...
![](../_assets/2.서보AMP/E02541/E02541_서보구동장치_POW_LED_위치_N제어기.PNG)

(a) Hi6-N 제어기 서보구동장치의 ‘POW LED’ 위치

![](../_assets/2.서보AMP/E02541/E02541_백플레인보드_LED5_위치_T제어기.PNG)

(b) Hi6-T 제어기 백플레인보드의 ‘LED5’ 위치

그림 1.1 제어기 전원표시 LED 위치
...
# 3.2. E0014. 안전스위치(EM, OTR, TS 등) 순간접촉
...
# 3.3. E0043. 자동모드 안전가드(Auto Guard) 스위치 접속 이상
...
# 3.6. E02201. 본체 리밋SW 입력불일치 (안전체인 1 OFF)
...
# 3.7. E02202. 본체 리밋SW 입력불일치 (안전체인 2 OFF)
...
# 3.9. E02207. 본체 리밋SW 입력불일치 (안전체인 1 OFF)
...
# 3.10. E02208. 본체 리밋SW 입력불일치 (안전체인 2 OFF)
...
# 3.12. E02261. 서보ON 상태에서 전자접촉기(MC2) 고장/검지이상

### 1. 개요

서보 ON 상태에서 마그네트(전자접촉기, magnetic contactor) MC2가 비정상적으로 OFF되었습니다.
...
세부적인 점검방법은 “E02260 서보ON 시도중 전자접촉기(MC2) 고장/검지 이상”을 참조하십시오.
...
# 3.14. E02281. 서보ON 상태에서 전자접촉기(MC1) 고장/검지이상
...
# 3.15. E2301. CPUERR 신호 불일치 (H6COM 테스크 에러)
...
# 3.16. E64002 H6COM-T Heartbeat 갱신 중단 에러
...
# 3.17. E64003 서보보드(BD640) 상태 입력 에러
...
# 3.18. E64035. 안전모듈 상태 출력(SFST) 불일치 (안전체인2 OFF)
