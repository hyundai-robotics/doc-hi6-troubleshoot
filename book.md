...
그림 1.1 CNDR에서 저항 값 측정# E02504 AMP 다이오드모듈 이상 또는 교류 입력전압 초과
...
그림 1.2 Hi6-T 제어기 과전압 발생 에러 관련 부품 배치# E02506 AMP PN 저전압 발생
...
*   Hi6-N제어기 : BD640
*   Hi6-T15제어기 : BD641T# E02522 (○축) IPM 폴트 – 특정스텝
...
# 3. 안전신호 보드# E00002. 하드웨어 리밋스위치 작동중
...
세부적인 점검방법은 "E02450 (O축) 엔코더 응답 없음"을 참조하십시오.# E02453. (O축) 엔코더 데이터 오류 (CRC 오류 검출됨)
...
# E02454. (O축) 엔코더 단선 혹은 접촉불량(모터 OFF 상태)
...
세부적인 점검방법은 "E02450 (O축) 엔코더 응답 없음"을 참조하십시오.# E02455. (O축) 엔코더 데이터 이상 (비정상 값 변화 감지)
...
세부적인 점검방법은 "E02450 (O축) 엔코더 응답 없음"을 참조하십시오.# E02459. (O축) 엔코더 단선 혹은 접촉불량(모터 ON 상태)
...
세부적인 점검방법은 "E02450 (O축) 엔코더 응답 없음"을 참조하십시오.# E02450. (O축) 엔코더 내부 회전값 오류 (CE 비트 감지)
...
![](../_assets/4.서보보드/로봇_모터_위치.png)
                        ...
                        (그림 4.28 로봇의 각 축 모터 위치)# E02461. (O축) 엔코더 과속 오류 (OS 비트 감지)
...
세부적인 점검방법은 "E02460 (O축) 엔코더 내부 회전값 오류(CE 비트 감지)"을 참조하십시오.# E02463. (O축) 엔코더 멀티턴 저장 오류 (ME 비트 감지)
...
세부적인 점검방법은 "E02630 (O축) 위치편차 초과"을 참조하십시오.# E02632. (O축) 위치편차초과 - 브레이크전압 저하
...
![](../_assets/4.서보보드/로봇모델확인.png)
    ...
    TP화면상의 등록된 로봇기종과 실제 설치된 로봇이 일치하는지 확인하십시오.# E02650. (O축) 모터 과부하
...
![](../_assets/4.서보보드/축구동부.png)
                    ...
                    (그림 4.100 구동부 정상동작 확인)# E02651. (O축) 모터 과부하 - 브레이크전압 저하
...
공정 내 배선 Short 발생으로 보드 24V 전원 및 그라운드 상태 이상으로 IO 신호들에 대한 에러가 다발로 발생할 수 있습니다. BD640 및 BD632의 전원 계통 (노이즈, 와이어하네스 배선 상태 등)의 점검이 필요합니다. # E02781. (O축) 서보락 유지 불가 - 파라미터 이상
...
![](../_assets/4.서보보드/T제어기_서보보드_교체.png)
                    ...
                    (그림 4.53 T제어기 서보보드 교체)# 5. 통신# E29003 전장 보드 통신 오류 (EtherCAT 연결 끊김)
